  ...
  return (
    <div onClick={handleNavigate} className={`${poolTheme ? 'pool_theme': ''} poolcard`}>
      ...
      <div className='pool_data'>
       <div>
        <p>Total Supply</p>
        <h5> {totalLiquidity !== undefined? '$'+ Number(totalLiquidity).toFixed(2): '$162,000' } </h5>
       </div>
       <div>
       <p>Total Borrowed</p>
        <h5>{totalBorrowed !== undefined? '$'+ Number(totalBorrowed).toFixed(2): '$102,000' }</h5>
       </div>
      </div>
    ...
    {
    poolTheme &&  <div className='footer_img'>
