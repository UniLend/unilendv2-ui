...
export function numberFormat(x, po) {
    ...
    if(parts[1]){
        if(po > 0){ parts[1] = parts[1].substring(0, po); } 
...
export function lendAPY(token) {
    div(
        token.borrowApy,
        div(token.totLiqFull, token.totalBorrow)
      )
}
...
export function getBorrowMax(selectedToken, poolData, LTV = poolData.ltv) {
  const lendToken = poolData?.token0.address === selectedToken.address ? poolData?.token1 : poolData?.token0;
  const borrowedToken = selectedToken;
  let result;
  result = Number(lendToken.lendBalanceFixed) * (Number(LTV)/100) - Number(borrowedToken.borrowBalanceFixed);
  return result;
}
...
export function getLTV(selectedToken, poolData, inputBorrow) {
    const lendToken = poolData?.token0.address === selectedToken.address ? poolData?.token1 : poolData?.token0;
    const borrowedToken = selectedToken;
    const prevLTV = Number(borrowedToken.borrowBalanceFixed) > 0 ? Number(borrowedToken.borrowBalanceFixed) / Number(lendToken.lendBalanceFixed): 0;
